...
Query.prototype.filter = function (filters) {
    ...
    //Order
    if (undefined !== filters.order) {
        debug('find: adding order %d', filters.order);
        let [field, desc] = filters.order.split(' ');
        const descending = desc.toUpperCase() === "DESC" || false;
        const modelIdName = this.definition.model.getIdName();
        field = field === modelIdName ? '__key__' : field

        this.query.order(field, { descending });
    }
};
...
Query.prototype.where = function (data, name) {
    ...
    if (this.idName == name) {
        ...
        key = this.ds.key([this.model, isNaN(data) ? data : parseInt(data)]);
    ...
    this.ds.runQuery(this.query, (error, result, cursor) => {
        if (result !== undefined && result.length > 0) {
            result = result.map((entity) => {
                ...
                entity[this.idName] = key[this.idName] || key['id'] || key['name'];
...
Query.prototype.findById = function (id, callback) {
    ...
    filter.where[this.idName] = isNaN(id) ? id : parseInt(id);
    this.filter(filter);
...
Query.prototype.update = function (id, data, callback) {
    this.findById(id, (error, entity) => {
        ...
        let key = this.ds.key([this.model, isNaN(id) ? id : parseInt(id)]);
        this.ds.update({
            key: key,
            data: _data
        }, (errors, result) => {
